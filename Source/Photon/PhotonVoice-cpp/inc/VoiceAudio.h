...
namespace ExitGames
{
	namespace Voice
	{
		...
		template<typename T>
		class IAudioPusher : public IAudioDesc, virtual public Common::ToString
		{
		public:
			...
			virtual void setCallback(void* opaque, void(*pCallback)(void*, const Buffer<T>&)) = 0;
		};
		...
		protected:
			LocalVoiceAudio(const Voice::Logger& logger, VoiceClient* pVoiceClient, nByte id, const VoiceInfo& voiceInfo, const IAudioDesc& audioSourceDesc, int channelId, const VoiceCreateOptions& options); // created by VoiceClient
		...
		{
		public:
			LocalVoiceAudioDummy(const Voice::Logger& logger);
