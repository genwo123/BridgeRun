...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		int MutableRoom::getPlayerCount(void) const
		{
		...
		template<typename Etype> bool MutableRoom::setRoomProperty(nByte key, const Etype pValueArray, int arrSize, const WebFlags& webflags)
		{
			Hashtable properties;
		...
		template<typename Etype> bool MutableRoom::setRoomProperty(nByte key, const Etype pValueArray, const int* pArrSizes, const WebFlags& webflags)
		{
			Hashtable properties;
		...
		bool MutableRoom::setMasterClient(const Player& masterClientPlayer)
		{
			if(masterClientPlayer.getNumber() == getMasterClientID())
				return false;
			Hashtable newProps, prevProps;
			newProps.put(Properties::Room::MASTER_CLIENT_ID, masterClientPlayer.getNumber());
			prevProps.put(Properties::Room::MASTER_CLIENT_ID, getMasterClientID());
			return mpLoadBalancingClient->opSetPropertiesOfRoom(newProps, prevProps);
		}
