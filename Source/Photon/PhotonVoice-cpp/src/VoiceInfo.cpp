...
namespace ExitGames
{
	namespace Voice
	{
		...
		/// <param name="userdata">Optional user data. Should be serializable by Photon.</param>
		/// <returns>VoiceInfo instance.</returns>
		VoiceInfo VoiceInfo::createVideoVP8(int bitrate, int width, int height, const Object& userdata)
		{
			return VoiceInfo()
		...
		Codec::Enum VoiceInfo::getCodec(void) const
		{
			return mCodec;
		}

		VoiceInfo& VoiceInfo::setCodec(Codec::Enum codec)
		{
			mCodec = codec;
			return *this;
		}
		...
		int VoiceInfo::getFPS(void) const
		{
			return mFPS;
		}

		VoiceInfo& VoiceInfo::setFPS(int fps)
		{
			mFPS = fps;
			return *this;
		}

		/// <summary>Video keyframe interval</summary>
		int VoiceInfo::getKeyFrameInt(void) const
		{
			return mKeyFrameInt;
		}

		VoiceInfo& VoiceInfo::setKeyFrameInt(int kfi)
		{
			mKeyFrameInt = kfi;
			return *this;
		}
