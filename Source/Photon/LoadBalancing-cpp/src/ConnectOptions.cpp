...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		/**
		   ...
		   @param tryUseDatagramEncryption Setting this to 'true' results in encryption-key exchange via an initial WSS-connection and in all UDP traffic getting encrypted, if all of
		   the following is true: the argument for 'serverType' is ServerType::NAME_SERVER, the connection protocol specified in the ClientConstructOptions is ConnectionProtocol::UDP, the Photon Client for the platform
		...
		ConnectOptions::ConnectOptions(const AuthenticationValues& authenticationValues, const JString& username, const JString& serverAddress, nByte serverType, bool tryUseDatagramEncryption, bool useBackgroundSendReceiveThread)
			: mAuthenticationValues(authenticationValues)
			, mUsername(username)
			, mServerAddress(serverAddress)
			, mServerType(serverType)
			, mTryUseDatagramEncryption(tryUseDatagramEncryption)
			, mUseBackgroundSendReceiveThread(useBackgroundSendReceiveThread)
		{
		}
		...
		/**
		   @returns the currently set value for the useBackgroundSendReceiveThread flag
		   @sa
		   setUseBackgroundSendReceiveThread() */
		bool ConnectOptions::getUseBackgroundSendReceiveThread(void) const
		{
			return mUseBackgroundSendReceiveThread;
		}

		/**
		   Sets the value for the useBackgroundSendReceiveThread flag.
		   @note 'useBackgroundSendReceiveThread' is currently only supported on the Nintendo Switch. On all other platforms it's value is ignored and the client always behaves as if it was 'false'.
		   @param useBackgroundSendReceiveThread when this is 'true', then all UDP/TCP data transmission will happen in a background thread, otherwise it will happen inside Client::serviceBasic() - optional, defaults to true.
		   @returns a reference to the instance on which it was called to allow for chaining multiple setter calls
		   @sa
		   getUseBackgroundSendReceiveThread() */
		ConnectOptions& ConnectOptions::setUseBackgroundSendReceiveThread(bool useBackgroundSendReceiveThread)
		{
			mUseBackgroundSendReceiveThread = useBackgroundSendReceiveThread;
			return *this;
		}
		...
		{
			return retStr += JString(L"{authenticationValues=") + getAuthenticationValues().toString(withTypes) + L", username=" + getUsername().toString(withTypes) + L", serverAddress=" + getServerAddress().toString(withTypes) + L", serverType=" + getServerType() + L", tryUseDatagramEncryption=" + getTryUseDatagramEncryption() + L", useBackgroundSendReceiveThread=" + getUseBackgroundSendReceiveThread() + L"}";
		}
	}
}