...
namespace ExitGames
{
	...
	{       /// <summary>Collection of Audio Utility functions and classes.</summary>
		namespace AudioUtil
		{
			template<typename T> void resample(const T* srcBuf, int srcOffsetSamples, int srcSamples, T* dstBuf, int dstOffsetSamples, int dstSamples, int dstChannels);
			template<typename T> void resample(const T* srcBuf, int srcOffsetSamples, int srcSamples, int srcChannels, T* dstBuf, int dstOffsetSamples, int dstSamples, int dstChannels);

			template<typename T> void resample(const Buffer<T>& srcBuf, Buffer<T>& dstBuf, int dstSamples, int channels);
			template<typename T> void resample(const Buffer<T>& srcBuf, int srcOffsetSamples, int srcSamples, Buffer<T>& dstBuf, int dstOffsetSamples, int dstSamples, int channels);
			template<typename T> void resample(const Buffer<T>& srcBuf, int srcOffsetSamples, int srcSamples, int srcChannels, Buffer<T>& dstBuf, int dstOffsetSamples, int dstSamples, int dstChannels);
			...
			template<typename T>
			Buffer<T> VoiceDetector<T>::mEmptyBuffer;
			...
			public:
				...
				int process(Buffer<T> s, Buffer<T> d);
				int end(Buffer<T> s);
			private:
				int mChannels;
				int mSkipGroup;
		...
		public:
			...
			virtual void start(int frequency, int channels, int frameSamples) = 0;
