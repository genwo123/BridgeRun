...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		/** @copydoc Peer::getSerializationProtocol() */
		nByte Client::getSerializationProtocol(void) const
		{
			return mpPeer->getSerializationProtocol();
		}
		...
		/**
		   ...
		   @param clientConstructOptions An instance of class ClientConstructOptions
		   @sa Listener, ConnectionProtocol, RegionSelectionMode, NetworkPort, SerializationProtocol */
		Client::Client(Listener& listener, const JString& applicationID, const JString& appVersion, const ClientConstructOptions& clientConstructOptions)
...
#	pragma warning(disable:4355)
			...
			, mpPeer(allocate<Peer>(static_cast<PhotonListener&>(*this), ConnectionProtocol::WSS, clientConstructOptions.getSerializationProtocol()))
#else
			, mpPeer(allocate<Peer>(static_cast<PhotonListener&>(*this), clientConstructOptions.getConnectionProtocol(), clientConstructOptions.getSerializationProtocol()))
#endif
			...
			, mAutoLobbyStats(clientConstructOptions.getAutoLobbyStats())
			, mDisconnectedCause(DisconnectCause::NONE)
			, M_REGION_SELECTION_MODE(clientConstructOptions.getRegionSelectionMode())
			, M_CONNECTION_PROTOCOL(clientConstructOptions.getConnectionProtocol())
#if defined EG_PLATFORM_SUPPORTS_CPP11 && defined EG_PLATFORM_SUPPORTS_MULTITHREADING
			, mPingsPerRegion(0)
#endif
#ifdef _EG_USE_WSS_ON_NS_BUT_SPECIFIED_PROTOCOL_ELSEWHERE
			...
			, mUseUDPEncryption(clientConstructOptions.getConnectionProtocol()==ConnectionProtocol::UDP&&mUseAuthOnce)
#elif !defined _EG_DATAGRAM_ENCRYPTOR_AVAILABLE || !defined _EG_WEBSOCKET_AVAILABLE
			, mUseUDPEncryption(false)
#endif
			, mUseAlternativePorts(clientConstructOptions.getUseAlternativePorts())
			, mpPuncherClient(allocate<PuncherClient>(*this, mListener, mLogger, clientConstructOptions.getSerializationProtocol()))
#ifdef EG_PARALLELLY_PINGEND_REGIONS_LIMIT
			, mBestRegionPinger(*allocate<BestRegionPinger>())
			, M_MAX_PARALLELLY_PINGEND_REGIONS(ConnectionProtocol::getIsWebSocket(M_CONNECTION_PROTOCOL)?4:0)
#endif
			, M_SERIALIZATION_PROTOCOL(clientConstructOptions.getSerializationProtocol())
...
#	pragma warning(pop)
#endif
		{
			...
			for(int p=-1; (p=mAppIDPeerInit.indexOf('-')) >= 0;)
		...
		bool Client::connect(const ConnectOptions& connectOptions)
		{
			if(mState > PeerStates::PeerCreated && mState < PeerState::DISCONNECTED)
			{
				EGLOG(DebugLevel::ERRORS, L"The client isn't in a disconnected state");
				return false;
			}
#ifdef _EG_USE_WSS_ON_NS_BUT_SPECIFIED_PROTOCOL_ELSEWHERE
			mpPeer->setConnectionProtocol(ConnectionProtocol::WSS);
#else
			mUseAuthOnce = !ConnectionProtocol::getIsWebSocket(M_CONNECTION_PROTOCOL) && connectOptions.getServerType()==ServerType::NAME_SERVER;
#	if defined _EG_DATAGRAM_ENCRYPTOR_AVAILABLE && defined _EG_WEBSOCKET_AVAILABLE
			mUseUDPEncryption = connectOptions.getTryUseDatagramEncryption()&&mUseAuthOnce&&M_CONNECTION_PROTOCOL==ConnectionProtocol::UDP;
			if(mUseUDPEncryption && connectOptions.getServerType() == ServerType::NAME_SERVER)
				mpPeer->setConnectionProtocol(ConnectionProtocol::WSS);
#	endif
#endif
#if defined _EG_EMSCRIPTEN_PLATFORM || defined _EG_WEBSOCKET_AVAILABLE
			const int index = connectOptions.getServerAddress().indexOf("://");
			const JString serverAddressWithoutProtocol = index==-1?connectOptions.getServerAddress():connectOptions.getServerAddress().substring(index+JString(L"://").length());
#else
			const JString& serverAddressWithoutProtocol = connectOptions.getServerAddress();
#endif
			if(mpPeer->connect(addPortToAddress(serverAddressWithoutProtocol, connectOptions.getServerType()), mAppIDPeerInit, mUseBackgroundSendReceiveThread=connectOptions.getUseBackgroundSendReceiveThread()))
			{
				JString userID;
				if(!connectOptions.getAuthenticationValues().getUserID().length())
					userID = mAuthenticationValues.getUserID();
				mAuthenticationValues = connectOptions.getAuthenticationValues();
				if(userID.length())
					mAuthenticationValues.setUserID(userID);
				getLocalPlayer().setName(connectOptions.getUsername());
				if(connectOptions.getServerType() == ServerType::MASTER_SERVER)
					mMasterserver = connectOptions.getServerAddress();
				mState = connectOptions.getServerType()==ServerType::NAME_SERVER?PeerStates::ConnectingToNameserver:PeerStates::Connecting;
				return true;
			}
			else
				return false;
		...
		void Client::disconnect(void)
		{
			if(mState <= PeerStates::PeerCreated && mState >= PeerState::DISCONNECTED)
			{
				EGLOG(DebugLevel::ERRORS, L"The client isn't in a connected state");
				return;
			}
			if(mState == PeerState::DISCONNECTING)
			{
				EGLOG(DebugLevel::ERRORS, L"Disconnection already in progress");
				return;
			}
		...
		/** @fn template<typename Ftype> bool Client::opRaiseEvent(bool reliable, const Ftype& parameters, nByte eventCode, const RaiseEventOptions& options)
		   ...
		   It is recommended to keep the payload as compact as possible, especially for events that get raised multiple times per second.
		...
		/** @fn template<typename Ftype> bool Client::opRaiseEvent(bool reliable, const Ftype pParameterArray, int arrSize, nByte eventCode, const RaiseEventOptions& options)
		   @overload
		...
		/** @fn template<typename Ftype> bool Client::opRaiseEvent(bool reliable, const Ftype pParameterArray, const int* pArrSizes, nByte eventCode, const RaiseEventOptions& options)
		   @overload
		   @param reliable true = operation will be sent reliably; false = no resend in case of packet loss - will be ignored, when not using UDP as protocol
		...
		/**
		   ...
		   A room will be attached to the lobby that you have specified in the provided options. Leave the lobby name empty to attach the room to the lobby in which you reside at the time of
		   room creation. If you are in no lobby, then the default lobby is used.
		...
		/**
		   ...
		   In the case that a new room gets created, that room will be attached to the lobby that you have specified in the provided options. Leave the lobby name empty to attach the room to the
		   lobby in which you reside at the time of room creation. If you are in no lobby, then the default lobby is used.
		...
		/**
		   ...
		   @returns true, if the request could successfully be queued for sending to the server, false otherwise.
		   @sa
		   opRaiseEvent(), RaiseEventOptions::setInterestGroup(), <a href="https://doc.photonengine.com/en-us/realtime/current/reference/interestgroups">Interestgroups</a> */
		...
		/** @fn template<typename Ftype> bool Client::opWebRpc(const Common::JString& uriPath, const Ftype& parameters, bool sendAuthCookie=false)
		   ...
		   @param sendAuthCookie defines if the authentication cookie gets sent to a WebHook (if set up)
		 */

		/** @fn template<typename Ftype> bool Client::opWebRpc(const Common::JString& uriPath, const Ftype pParameterArray, int arrSize, bool sendAuthCookie=false)
		   @overload
		   @param uriPath the URL path to call, relative to the baseUrl configured on Photon's server-side
		...
		/** @fn template<typename Ftype> bool Client::opWebRpc(const Common::JString& uriPath, const Ftype pParameterArray, const int* pArrSizes, bool sendAuthCookie=false)
		   @overload
		   @param uriPath the URL path to call, relative to the baseUrl configured on Photon's server-side
...
#	pragma GCC diagnostic pop
		...
		/** @fn template<typename Ftype> bool Client::sendDirect(const Ftype& parameters, const SendDirectOptions& options=SendDirectOptions())
		   ...
		   It is recommended to keep the payload as compact as possible, as the data is typically sent multiple times per second.
		   This easily adds up to a huge amount of data otherwise.
		   @note
		...
		/** @fn template<typename Ftype> int Client::sendDirect(const Ftype pParameterArray, int arrSize, const SendDirectOptions& options=SendDirectOptions())
		   @overload
		...
		/** @fn template<typename Ftype> int Client::sendDirect(const Ftype pParameterArray, const int* pArrSizes, const SendDirectOptions& options=SendDirectOptions())
		   @overload
		...
		int Client::sendDirect(const JVector<nByte>& buffer, const SendDirectOptions& options)
		{
			if(!getIsInGameRoom())
			{
				EGLOG(DebugLevel::ERRORS, L"no gameroom is currently joined");
			...
			if(options.getTargetPlayers().getSize())
			{
				for(unsigned int i=0; i<options.getTargetPlayers().getSize(); ++i)
				{
					int nr = options.getTargetPlayers()[i];
					// check if requested player joined
					// TODO: cache joined players numbers in Client in sorted array
					for(nByte j=0; j<getCurrentlyJoinedRoom().getPlayerCount(); ++j) 
						if(nr == getCurrentlyJoinedRoom().getPlayers()[j]->getNumber())
							availablePlayers.addElement(nr);
				}
			}
			else if(options.getReceiverGroup() == ReceiverGroup::MASTER_CLIENT)
				availablePlayers.addElement(getCurrentlyJoinedRoom().getMasterClientID());
			else
			{
				for(nByte i=0; i<getCurrentlyJoinedRoom().getPlayerCount(); ++i) 
				{
					int nr = getCurrentlyJoinedRoom().getPlayers()[i]->getNumber();
					if((options.getReceiverGroup() == ReceiverGroup::OTHERS && nr != localPlayer) || (options.getReceiverGroup() == ReceiverGroup::ALL))
						availablePlayers.addElement(nr);
				}
			}
			int sent = mpPuncherClient->sendDirect(buffer, availablePlayers, options.getFallbackRelay());
			return sent;
		}
		...
		{
			int debugLevel = operationResponse.getReturnCode()?DebugLevel::ERRORS:DebugLevel::INFO;
			if(getDebugOutputLevel() >= debugLevel)
				EGLOG(debugLevel, operationResponse.toString(true, operationResponse.getOperationCode()==OperationCode::AUTHENTICATE||operationResponse.getOperationCode()==OperationCode::AUTH_ONCE));
			...
			if(operationResponse.getReturnCode() == ErrorCode::ApplicationLayer::OPERATION_LIMIT_REACHED)
			{
				EGLOG(DebugLevel::ERRORS, L"The client was disconnected because the operation limit was reached. Errorcode %d: %ls", operationResponse.getReturnCode(), operationResponse.getDebugMessage().cstr());
				mDisconnectedCause = DisconnectCause::DISCONNECT_BY_OPERATION_LIMIT;
				disconnect();
			}
			...
			case OperationCode::AUTH_ONCE:
				{
					if(operationResponse.getReturnCode())
					{
						...
						{
						case ErrorCode::ApplicationLayer::INVALID_AUTHENTICATION:
							mDisconnectedCause = DisconnectCause::INVALID_AUTHENTICATION;
							break;
						case ErrorCode::ApplicationLayer::CUSTOM_AUTHENTICATION_FAILED:
							mDisconnectedCause = DisconnectCause::CUSTOM_AUTHENTICATION_FAILED;
							break;
						case ErrorCode::ApplicationLayer::AUTHENTICATION_TICKET_EXPIRED:
							mDisconnectedCause = DisconnectCause::AUTHENTICATION_TICKET_EXPIRED;
							break;
						case ErrorCode::ApplicationLayer::INVALID_REGION:
							mDisconnectedCause = DisconnectCause::INVALID_REGION;
							break;
						case ErrorCode::ApplicationLayer::MAX_CCU_REACHED:
							mDisconnectedCause = DisconnectCause::MAX_CCU_REACHED;
							break;
						case ErrorCode::Core::OPERATION_DENIED:
							mDisconnectedCause = DisconnectCause::OPERATION_NOT_ALLOWED_IN_CURRENT_STATE;
							break;
						case ErrorCode::ApplicationLayer::CLIENT_VERSION_TOO_OLD:
							mDisconnectedCause = DisconnectCause::CLIENT_VERSION_TOO_OLD;
							break;
						case ErrorCode::ApplicationLayer::CLIENT_VERSION_INVALID:
							mDisconnectedCause = DisconnectCause::CLIENT_VERSION_INVALID;
							break;
						case ErrorCode::ApplicationLayer::DASHBOARD_VERSION_INVALID:
							mDisconnectedCause = DisconnectCause::DASHBOARD_VERSION_INVALID;
							break;
						}
					...
					else
					{
						if(mState == PeerStates::ConnectedToNameserver || mState == PeerStates::Connected || mState == PeerStates::ConnectedComingFromGameserver)
						{
							if(operationResponse.getParameters().contains(ParameterCode::USER_ID))
							{
								...
								if(mpCurrentlyJoinedRoom)
									PlayerMovementInformant::onLeaveLocal(*mpCurrentlyJoinedRoom);
								...
								destroyMutablePlayer(mpLocalPlayer);
								mpLocalPlayer = createMutablePlayer(-1, properties);
								if(getDebugOutputLevel() >= DebugLevel::INFO)
						...
						if(mState == PeerStates::ConnectedToNameserver)
						{
							if(operationResponse.getParameters().contains(ParameterCode::ENCRYPTION_DATA))
							{
								...
								break;
								case EncryptionMode::UDP_TRAFFIC_ENCRYPTION_RANDOM_SEQUENCE_GCM:
								{
#ifdef _EG_DATAGRAM_ENCRYPTOR_AVAILABLE
									...
									mpPeer->initUDPEncryption(encryptSecret);
#else
			...
			case OperationCode::JOIN_ROOM:
				{
					if(getIsOnGameServer())
					{
						...
						int playerListSize = *ValueObject<int*>(operationResponse.getParameterForCode(ParameterCode::PLAYER_LIST)).getSizes();
			...
			case OperationCode::GET_ROOM_LIST:
				...
				break;
			case OperationCode::SET_PROPERTIES:
			{
				if(operationResponse.getReturnCode())
					mListener.onPropertiesChangeFailed();
				break;
			}
			break;
			default:
		...
		void Client::onStatusChanged(int statusCode)
		{
			...
			case StatusCode::DISCONNECT:
				{
					...
					{
						// set specified protocol again after temporal usage of wss for name server connection
			...
			case StatusCode::QUEUE_SENT_WARNING:
				...
				break;
			case ErrorCode::Core::OPERATION_INVALID:
			case ErrorCode::Core::INTERNAL_SERVER_ERROR:
		...
		{
			int debugLevel = eventData.getCode()>EventCode::MAX_CUSTOM_EVCODE&&eventData.getCode()!=EventCode::PUNCH_MSG&&eventData.getCode()!=EventCode::VOICE_FRAME_DATA?DebugLevel::INFO:DebugLevel::ALL;
			if(getDebugOutputLevel() >= debugLevel)
				EGLOG(debugLevel, L"%ls", eventData.toString().cstr()); // don't print out the payload here, as that can get too expensive for big events
			...
			case EventCode::JOIN:
				{
					...
					case DirectMode::ALL_TO_OTHERS:
						if(nr != getLocalPlayer().getNumber())
							startPunch(nr);
						break;
					case DirectMode::MASTER_TO_OTHERS:
						if(getLocalPlayer().getIsMasterClient() && nr != getLocalPlayer().getNumber())
							startPunch(nr);
						break;
...
#if defined EG_PLATFORM_SUPPORTS_CPP11 && defined EG_PLATFORM_SUPPORTS_MULTITHREADING
		void Client::onPingResponse(const JString& address, unsigned int result, unsigned int pingSendTime)
		{
			EGLOG(DebugLevel::INFO, L"adress %ls: ping result: %u, sendTime: %d", address.cstr(), result, pingSendTime);
			...
			for(unsigned int i=0; i<mAvailableRegionServers.getSize(); ++i)
			{
				...
				if(address == mAvailableRegionServers[i])
					mPingSendTimes[i].addElement(pingSendTime);
			...
			for(unsigned int i=0; i<mPingResponses.getSize(); ++i)
			{
				...
				for(unsigned int j=0; j<mPingsPerRegion; ++j)
				{
					...
					resultsString += JString(L" ") + mPingResponses[i][j];
					timesString += JString(L" ") + mPingSendTimes[i][j];
				}
				ping /= mPingsPerRegion;
				EGLOG(DebugLevel::WARNINGS, L"average ping to region %ls: %u, individual ping results:%ls, ping send times:%ls", mAvailableRegions[i].cstr(), ping, resultsString.cstr(), timesString.cstr());
				if(ping < bestPing)
				{
					bestPing = ping;
			...
			mPingSendTimes.removeAllElements();
...
#if defined EG_PLATFORM_SUPPORTS_CPP11 && defined EG_PLATFORM_SUPPORTS_MULTITHREADING
		void Client::pingBestRegion(void)
		{
#	ifdef EG_PARALLELLY_PINGEND_REGIONS_LIMIT
			if(!M_MAX_PARALLELLY_PINGEND_REGIONS)
			{
#	endif
				EGLOG(DebugLevel::INFO, L"");
				for(unsigned int i=0; i<mAvailableRegionServers.getSize(); ++i)
				{
					mpPeer->pingServer(mAvailableRegionServers[i], M_PINGS_PER_REGION, M_CONNECTION_PROTOCOL);
					mPingResponses.addElement(JVector<unsigned int>(M_PINGS_PER_REGION));
					mPingSendTimes.addElement(JVector<int>(M_PINGS_PER_REGION));
				}
				mPingsPerRegion = M_PINGS_PER_REGION;
#	ifdef EG_PARALLELLY_PINGEND_REGIONS_LIMIT
			}
			else
			{
				...
				mBestRegionPinger.retain();
				Thread::create(BestRegionPinger::pingBestRegion, this);
			}
#	endif
		}

#	ifdef EG_PARALLELLY_PINGEND_REGIONS_LIMIT
		Client::BestRegionPinger::BestRegionPinger(void)
			: mRefCount(0)
		{
			retain();
		}
		...
		const Client::BestRegionPinger* Client::BestRegionPinger::retain(void) const
		{
			++mRefCount;
			return this;
		}

		void Client::BestRegionPinger::release(void) const
		{
			if(!--mRefCount)
				DEALLOCATE(BestRegionPinger, this);
		}

		void Client::BestRegionPinger::pingBestRegion(void* pArg)
		{
			Client* pClient = reinterpret_cast<Client*>(pArg);
			Logger& mLogger = pClient->mLogger;
			EGLOG(DebugLevel::INFO, L"");
			pClient->mPingsPerRegion = M_PINGS_PER_REGION;
			unsigned int regionsPinged = 0, instancePingsInProgress = 0;
#		ifdef EG_PLATFORM_SUPPORTS_ATOMICS
			static LockableNonAssignable<std::atomic<unsigned int> > pingsInProgress = 0;
#		else
			static Lockable<unsigned int> pingsInProgress = 0;
#		endif

			while(regionsPinged<pClient->mAvailableRegionServers.getSize() || instancePingsInProgress)
			{
				if(instancePingsInProgress)
				{
					unsigned int oldInstancePingsInProgress = instancePingsInProgress;
					instancePingsInProgress = 0;
					for(unsigned int i=0; i<regionsPinged; ++i)
						if(pClient->mPingResponses[i].getSize()<M_PINGS_PER_REGION)
							++instancePingsInProgress;
					pingsInProgress -= oldInstancePingsInProgress-instancePingsInProgress;
				}
				if(pingsInProgress<pClient->M_MAX_PARALLELLY_PINGEND_REGIONS)
				{
					std::lock_guard<std::mutex> lock(pingsInProgress);
					for(unsigned int i=regionsPinged; i<pClient->mAvailableRegionServers.getSize() && pingsInProgress<pClient->M_MAX_PARALLELLY_PINGEND_REGIONS; ++i)
					{
						pClient->mpPeer->pingServer(pClient->mAvailableRegionServers[i], M_PINGS_PER_REGION, pClient->M_CONNECTION_PROTOCOL);
						++regionsPinged;
						++pingsInProgress;
						++instancePingsInProgress;
					}
			...
			pClient->mBestRegionPinger.release();
		}
#	endif
#endif
		...
		{
			if(mUseAuthOnce)
				return mpPeer->connect(address, mAppIDPeerInit, mAuthenticationValues.getSecret(), mUseBackgroundSendReceiveThread);
			else
		...
		{
			if(mUseAuthOnce)
				return mpPeer->opAuthenticateOnce(mAppID, mAppVersion, M_CONNECTION_PROTOCOL, mUseUDPEncryption?EncryptionMode::UDP_TRAFFIC_ENCRYPTION_RANDOM_SEQUENCE_GCM:EncryptionMode::USER_DATA_ENCRYPTION_NO_DIFFIE_HELLMAN, mAuthenticationValues, mAutoLobbyStats&&mState!=PeerStates::ConnectedToNameserver, mSelectedRegion);
			else
				return mpPeer->opAuthenticate(mAppID, mAppVersion, !mAuthenticationValues.getSecret().length(), mAuthenticationValues, mAutoLobbyStats&&mState!=PeerStates::ConnectedToNameserver, mSelectedRegion);  // the secret token already is encrypted itself, so there is no need to send the operation encrypted in the case of authentication by token
		}
...
#else
			switch(mUseUDPEncryption&&serverType==ServerType::NAME_SERVER ? mpPeer->getConnectionProtocol() : M_CONNECTION_PROTOCOL)
			{
#ifndef _EG_EMSCRIPTEN_PLATFORM
		...
		{
#ifndef _EG_EMSCRIPTEN_PLATFORM
			if((mpCurrentlyJoinedRoom->getDirectMode() == DirectMode::MASTER_TO_OTHERS && id != getLocalPlayer().getNumber()) || mpCurrentlyJoinedRoom->getDirectMode() == DirectMode::MASTER_TO_ALL) // punch from others/all to master
					startPunch(id);
#endif
			mListener.onMasterClientChanged(id, oldID);
		}

		bool Client::opRaiseEvent(bool reliable, const Common::Object& parameters, nByte eventCode, const RaiseEventOptions& options)
		{
			if(!getIsInGameRoom())
			{
				EGLOG(DebugLevel::ERRORS, L"no gameroom is currently joined");
		...
		bool Client::setMasterClient(const Player& masterClientPlayer)
		{
			Hashtable newProps, prevProps;
			newProps.put(Properties::Room::MASTER_CLIENT_ID, masterClientPlayer.getNumber());
			prevProps.put(Properties::Room::MASTER_CLIENT_ID, getLocalPlayer().getNumber());
			return opSetPropertiesOfRoom(newProps, prevProps);
		}
	}
}