...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		/**
		   Copy-Constructor: Creates a new instance that is a deep copy of the argument instance.
		   @param toCopy The instance to copy. */
		SendDirectOptions::SendDirectOptions(const RaiseEventOptions& toCopy)
		{
			*this = toCopy;
		}
		...
		/**
		   ...
		   This overwrites old data in the left operand. */
		SendDirectOptions& SendDirectOptions::operator=(const RaiseEventOptions& toCopy)
		{
			mTargetPlayers = JVector<int>(toCopy.getTargetPlayers(), toCopy.getNumTargetPlayers());
			mReceiverGroup = toCopy.getReceiverGroup();
