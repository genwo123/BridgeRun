...
namespace ExitGames
{
	namespace Voice
	{
		...
		public:
			...
			/// <summary>Closes the encoder. The encoder must not produce any output after this call. Called in LocalVoice destructor.</summary>
			...
			virtual void setOutput(void* opaque, void(*output)(void*, const Buffer<nByte>&, FrameFlags::Enum flags)) = 0;
			/// <summary>Returns next encoded data frame (if such output supported).</summary>
			virtual Buffer<nByte> dequeueOutput(FrameFlags::Enum& flags) = 0;
		};
		...
		public:
			...
			/// <summary>Closes the decoder. It will no longer receive any input after this call. Called by RemoteVoice during removal.</summary>
			virtual void close(void) = 0;
			...
			virtual void input(const FrameBuffer& buf) = 0;
		};
		...
		{
		public:
			UnsupportedCodecError(const Common::JString& info, int codec, const Voice::Logger& logger);

			virtual void close(void);
			virtual Common::JString getError(void) const;
			virtual void open(const VoiceInfo& info);
			virtual void setOutput(void* opaque, void(*output)(void*, const Buffer<nByte>&, FrameFlags::Enum));
