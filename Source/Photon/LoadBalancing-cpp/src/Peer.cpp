...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		Peer::Peer(Photon::PhotonListener& listener, nByte connectionProtocol, nByte serializationProtocol)
			: PhotonPeer(listener, connectionProtocol, serializationProtocol)
		{
		}
		...
		bool Peer::opRaiseEvent(bool reliable, const Common::Object& parameters, nByte eventCode, const RaiseEventOptions& options)
		{
			...
			return opCustom(OperationRequest(OperationCode::RAISE_EVENT, op), reliable, options.getChannelID(), options.getEncrypt());
		}
		...
		{
			OperationRequestParameters op = enterRoomImplementation(&options, customLocalPlayerProperties, expectedUsers, false);
			if(gameID.length())
		...
		{
			OperationRequestParameters op = enterRoomImplementation(createIfNotExists?&options:NULL, customLocalPlayerProperties, expectedUsers, false);
			op.put(ParameterCode::ROOM_NAME, ValueObject<JString>(gameID));
		...
		{
			OperationRequestParameters op = enterRoomImplementation(createIfNotExists?&options:NULL, Hashtable(), expectedUsers, true);
			if(gameID.length())
				op.put(ParameterCode::ROOM_NAME, ValueObject<JString>(gameID));
			if(createIfNotExists)
				op.put(ParameterCode::JOIN_MODE, ValueObject<nByte>(JoinMode::CREATE_IF_NOT_EXISTS));
		...
		OperationRequestParameters Peer::enterRoomImplementation(const RoomOptions* pOptions, const Hashtable& customLocalPlayerProperties, const JVector<JString>& expectedUsers, bool usePropertiesInsteadOfRoomProperties)
		{
			OperationRequestParameters op;
			...
			if(pOptions)
			{
				...
				// intentionally sending the array even when it is empty, as not sending the parameter at all means "list all properties in the lobby", while an empty array means "don't list any properties in the lobby"
				...
				if(roomProps.getSize())
					op.put(usePropertiesInsteadOfRoomProperties?ParameterCode::PROPERTIES:ParameterCode::ROOM_PROPERTIES, ValueObject<Hashtable>(roomProps));
