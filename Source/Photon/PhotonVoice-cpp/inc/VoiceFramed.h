...
namespace ExitGames
{
	namespace Voice
	{
		...
		protected:
			LocalVoiceFramed(const Voice::Logger& logger, VoiceClient* pVoiceClient, nByte id, const VoiceInfo& voiceInfo, int frameSize, int channelId, const VoiceCreateOptions& options = VoiceCreateOptions());
		private:
			int mFrameSize;
			Framer<T> mFramer;
			...
			std::atomic<bool> mEncodeThreadRunning;
