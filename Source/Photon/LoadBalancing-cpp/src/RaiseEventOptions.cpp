...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		/**
		   ...
		   @param encrypt see setEncrypt() - optional, defaults to false */
		RaiseEventOptions::RaiseEventOptions(nByte channelID, nByte eventCaching, const int* targetPlayers, short numTargetPlayers, nByte receiverGroup, nByte interestGroup, const WebFlags& webFlags, int cacheSliceIndex, bool encrypt)
		...
		int RaiseEventOptions::getNumTargetPlayers(void) const
		{
		...
		RaiseEventOptions& RaiseEventOptions::setTargetPlayers(const int* targetPlayers, int numTargetPlayers)
		{
		...
		/**
		   @returns the currently set value of the encrypt flag
		   @sa
		   setEncrypt() */
		bool RaiseEventOptions::getEncrypt(void) const
		{
			return mEncrypt;
		}

		/**
		   Sets if the event should be sent encrypted or unencrypted.
		   @details
		   Pass 'true' to sent the event encrypted or 'false' to send it unencrypted.
		   @note
		   This option is only relevant when neither 'true' has been set for ConnectOptions::setTryUseDatagramEncryption() nor ConnectionProtocol::WSS has been set for ClientConstructOptions::setConnectionProtocol().
		   Otherwise the value of this option gets ignored and all messages are always sent encrypted independently of the value of this option.
		   @remarks
		   This option uses payload encryption, which is less secure than datagram encryption and WSS.
		   It is stronlgy recommended for highly security-critical applications to either use ConnectOptions::setTryUseDatagramEncryption() or to pass ConnectionProtocol::WSS for ClientConstructOptions::setConnectionProtocol().
		   @param encrypt 'true' to sent the event encrypted or 'false' to send it unencrypted
		   @returns a reference to the instance on which it was called to allow for chaining multiple setter calls
		   @sa
		   getEncrypt() */
		RaiseEventOptions& RaiseEventOptions::setEncrypt(bool encrypt)
		{
			mEncrypt = encrypt;
			return *this;
		}
