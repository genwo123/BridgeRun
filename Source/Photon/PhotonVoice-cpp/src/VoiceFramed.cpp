...
namespace ExitGames
{
	namespace Voice
	{
		...
		template<typename T>
		LocalVoiceFramed<T>::LocalVoiceFramed(const Voice::Logger& logger, VoiceClient* pVoiceClient, nByte id, const VoiceInfo& voiceInfo, int frameSize, int channelId, const VoiceCreateOptions& options)
			: LocalVoice(logger, pVoiceClient, id, voiceInfo, channelId, options)
		...
		void LocalVoiceFramed<T>::pushDataAsync(const Buffer<T>& buf)
		{
			...
			if(!mEncodeThreadRunning)
			{
				...
				EGLOG(DebugLevel::INFO, getLogPrefix() + L": Starting data encode thread");
			...
			else
			{
				...
				{
					EGLOG(DebugLevel::WARNINGS, getLogPrefix() + L": PushData queue overflow. Frames skipped: " + (mFramesSkipped + 1));
		...
		void LocalVoiceFramed<T>::pushDataAsyncThread(void)
		{
			...
			EGLOG(DebugLevel::INFO, getLogPrefix() + L": Exiting data encode thread");
			mEncodeThreadRunning = false;
		}
