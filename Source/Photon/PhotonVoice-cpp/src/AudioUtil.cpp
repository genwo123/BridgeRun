...
namespace ExitGames
{
	namespace Voice
	{
		...
		namespace AudioUtil
		{
			...
			{
				resample(srcBuf.getArray(), srcOffsetSamples, srcSamples, dstBuf.getArray(), dstOffsetSamples, dstSamples, channels);
			}

			template<typename T>
			void resample(const T* srcBuf, int srcOffsetSamples, int srcSamples, T* dstBuf, int dstOffsetSamples, int dstSamples, int channels)
			{
				const T* src = srcBuf + srcOffsetSamples * channels;
				T* dst = dstBuf + dstOffsetSamples * channels;
			...
			{
				resample(srcBuf.getArray(), srcOffsetSamples, srcSamples, srcChannels, dstBuf.getArray(), dstOffsetSamples, dstSamples, dstChannels);
			}

			template<typename T>
			void resample(const T* srcBuf, int srcOffsetSamples, int srcSamples, int srcChannels, T* dstBuf, int dstOffsetSamples, int dstSamples, int dstChannels)
			{
				...
				const T* src = srcBuf + srcOffsetSamples * srcChannels;
				T* dst = dstBuf + dstOffsetSamples * dstChannels;
			...
			Buffer<T> VoiceDetector<T>::process(Buffer<T> buffer)
			{
				if(mOn)
				{
					...
					return mDetected ? buffer : mEmptyBuffer;
				}
				else
					return buffer;
			}
