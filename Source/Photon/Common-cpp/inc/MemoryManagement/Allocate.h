...
#include "Common-cpp/inc/MemoryManagement/Internal/ConstructArrayElements.h"
...
#			define ALLOCATE(type, p, ...) (p = reinterpret_cast<type*>(ExitGames::Common::MemoryManagement::EG_MALLOC(sizeof(type))), new(p) type(__VA_ARGS__))
			...
			/**
			   ...
			   (friend-/subclass-)access.
			   @note
			   For primitive types (non-class types) the template version is faster than the macro version, internally calls a partial specialization that is optimized for plain old data.
			   @param type the data type of the instance to create
			   @param p a pointer, in which the macro will store the address of the freshly created instance
...
#			define DEALLOCATE(type, p) if(p){p->~type(); ExitGames::Common::MemoryManagement::EG_FREE(const_cast<type*>(p));}
			...
			/**
			   ...
			   @param type the data type of the instance, to which p points
			   @param p a pointer to the instance to destroy */
#			define DEALLOCATE_ARRAY(type, p) if(p){EG_SIZE_T* pRaw = (reinterpret_cast<EG_SIZE_T*>(const_cast<type*>(p))-1); for(EG_SIZE_T i=*pRaw; i-->0;) p[i].~type(); ExitGames::Common::MemoryManagement::EG_FREE(pRaw);}

			/** @cond */
#			define ALLOCATE_TEMPLATE_BODY(...) Ftype* p; ALLOCATE(Ftype, p, __VA_ARGS__); return p;
#			define ALLOCATE_ARRAY_TEMPLATE_BODY(count, ...) Ftype* p; p = reinterpret_cast<Ftype*>(reinterpret_cast<EG_SIZE_T*>(EG_MALLOC(sizeof(Ftype)*count+sizeof(EG_SIZE_T)))+1); *(reinterpret_cast<EG_SIZE_T*>(p)-1) = count; Internal::constructArrayElements(p, count, ##__VA_ARGS__); return p;
#			define REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, ...) {Ftype* pOut; EG_SIZE_T oldCount = p?*(reinterpret_cast<EG_SIZE_T*>(const_cast<Ftype*>(p))-1):0; pOut = reinterpret_cast<Ftype*>(reinterpret_cast<EG_SIZE_T*>(EG_MALLOC(sizeof(Ftype)*count+sizeof(EG_SIZE_T)))+1); *(reinterpret_cast<EG_SIZE_T*>(pOut)-1) = count; Internal::constructArrayElements(pOut, Helpers::MathHelper::min(oldCount, static_cast<EG_SIZE_T>(count)), p); if(static_cast<EG_SIZE_T>(count)>oldCount) Internal::constructArrayElements(pOut+oldCount, static_cast<EG_SIZE_T>(count)-oldCount, ##__VA_ARGS__); DEALLOCATE_ARRAY(Ftype, p); p = pOut;} return p;
			/** @endcond */
...
EG_PRAGMA_VISIBILITY_PUSH_HIDDEN
			...
			template<typename Ftype>                                                                  const Ftype* reallocateArray(const Ftype* p, size_t count)                                                                       {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count)}
			template<typename Ftype, typename P1>                                                     const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1)                                                         {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1)}
			template<typename Ftype, typename P1>                                                     const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1)                                                         {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1)}
			template<typename Ftype, typename P1, typename P2>                                        const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2)                                           {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2)}
			template<typename Ftype, typename P1, typename P2>                                        const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2)                                           {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2)}
			template<typename Ftype, typename P1, typename P2>                                        const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2)                                           {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2)}
			template<typename Ftype, typename P1, typename P2>                                        const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2)                                           {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3>                           const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3)                             {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3,       P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4>              const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3, const P4& p4)               {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2,       P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1,       P2& p2, const P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2,       P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count,       P1& p1, const P2& p2, const P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2,       P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1,       P2& p2, const P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2,       P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3,       P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3,       P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3, const P4& p4,       P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			template<typename Ftype, typename P1, typename P2, typename P3, typename P4, typename P5> const Ftype* reallocateArray(const Ftype* p, size_t count, const P1& p1, const P2& p2, const P3& p3, const P4& p4, const P5& p5) {REALLOCATE_ARRAY_TEMPLATE_BODY(p, count, p1, p2, p3, p4, p5)}
			/** @endcond */
			...
			void deallocateArray(const Ftype* p)
			{
				if(p)
				{
					EG_SIZE_T* pRaw = (reinterpret_cast<EG_SIZE_T*>(const_cast<Ftype*>(p))-1);
					Internal::destructArrayElements(p, *pRaw);
					EG_FREE(pRaw);
				}
			}
