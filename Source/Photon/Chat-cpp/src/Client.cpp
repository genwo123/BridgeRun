...
namespace ExitGames
{
	namespace Chat
	{
		...
		/**
		   ...
		   @param connectionProtocol The protocol to use to connect to %Photon. Must match one of the constants specified in ConnectionProtocol.
		   @param serializationProtocol The protocol to use for serialization and deserialization of data. Must match one of the constants specified in SerializationProtocol*/
		Client::Client(Listener& listener, const JString& applicationID, const JString& appVersion, nByte connectionProtocol, nByte serializationProtocol)
#if defined _EG_MS_COMPILER
#	pragma warning(push)
#	pragma warning(disable:4355)
#endif
			: mPeer(*this, connectionProtocol, serializationProtocol)
			, mListener(listener)
...
#	pragma warning(pop)
		...
		/** @fn template<typename Ftype> bool Client::opPublishMessage(const Common::JString& channelName, const Ftype pMessageArray, int arrSize)
		    @overload
		    @param channelName channel name
		...
		/** @fn template<typename Ftype> bool Client::opPublishMessage(const Common::JString& channelName, const Ftype pMessageArray, const int* pArrSizes);
			@overload
		    @param channelName channel name
		    @param pMessageArray message to send
		...
		/** @fn template<typename Ftype> bool Client::opSendPrivateMessage(const Common::JString& userName, const Ftype pMessageArray, int arrSize, bool encrypt)
		    @overload
		    @param userName user name
		    @param pMessageArray message to send
			@param arrSize the number of elements in pParameterArray
		...
		/** @fn template<typename Ftype> bool Client::opSendPrivateMessage(const Common::JString& userName, const Ftype pMessageArray, const int* pArrSizes, bool encrypt)
			@overload
		    @param userName user name
		    @param pMessageArray message to send
			@param pArrSizes an array holding the number of elements for each dimension of pParameterArray
		...
		/** @fn template<typename Ftype> bool Client::opSetOnlineStatus(int status, const Ftype pMessageArray, int arrSize)
		    @overload
		    @param status predefined states are in namespace UserStatus. Other values can be used at will
		...
		/** @fn template<typename Ftype> bool Client::opSetOnlineStatus(int status, const Ftype pMessageArray, const int* pArrSizes)
		    @overload
		    @param status predefined states are in namespace UserStatus. Other values can be used at will
		    @param pMessageArray optional status message
			@param pArrSizes an array holding the number of elements for each dimension of pParameterArray
		...
		/**
		   ...
		   @note 'useBackgroundSendReceiveThread' is currently only supported on the Nintendo Switch. On all other platforms it's value is ignored and the client always behaves as if it was 'false'.
		   ...
		   @param useBackgroundSendReceiveThread when this is 'true', then all UDP/TCP data transmission will happen in a background thread, otherwise it will happen inside Client::serviceBasic() - optional, defaults to true.*/
		bool Client::connect(const AuthenticationValues& authenticationValues, const JString& nameServerAddress, bool useBackgroundSendReceiveThread)
		{
			JString userID;
...
#else
			const int port =
				ConnectionProtocol::getIsWebSocket(M_CONNECTION_PROTOCOL)?ConnectionProtocol::getIsSecure(M_CONNECTION_PROTOCOL)?NetworkPort::WSS::NAME:NetworkPort::WS::NAME
				:ConnectionProtocol::getIsTCP(M_CONNECTION_PROTOCOL)?NetworkPort::TCP::NAME
				:NetworkPort::UDP::NAME;
			const JString& nameServerAddressWithoutProtocol = nameServerAddress;
#endif
			if(mPeer.connect(nameServerAddressWithoutProtocol + (nameServerAddressWithoutProtocol.indexOf(L':') == -1?JString(L":")+(port):JString()), mAppIDPeerInit, useBackgroundSendReceiveThread))
			{
