...
namespace ExitGames
{
	namespace Voice
	{
		...
		int AudioOutDelayConfig::getLow(void) const
		{
			return mLow;
		}

		int AudioOutDelayConfig::getHigh(void) const
		{
			return mHigh;
		}
		int AudioOutDelayConfig::getMax(void) const
		{
			return mMax;
		}

		int AudioOutDelayConfig::getSpeedUpPerc(void) const
		{
			return mSpeedUpPerc;
		}

		template<typename T>
		AudioOutDelayControl<T>::AudioOutDelayControl(const Voice::Logger& logger, AudioOutDelayConfig delayConfig, const Common::JString& logPrefix, bool debugInfo)
			: mLogger(logger)
		...
		template<typename T>
		bool AudioOutDelayControl<T>::isFlushed(void) const
		{
			return !mStarted || mFlushed;
		}

		template<typename T>
		bool AudioOutDelayControl<T>::isPlaying(void) const
		{
		...
		void AudioOutDelayControl<T>::start(int frequency, int channels, int frameSamples)
		{
			...
			outStart();
			mStarted = true;
			EGLOG(DebugLevel::INFO, mLogPrefix + L" Start: " + (sizeof(T) == 2 ? L"short" : L"float") + L" bs=" + mBufferSamples + L" ch=" + channels + L" f=" + frequency + L" tds=" + mTargetDelaySamples + L" utds=" + mUpperTargetDelaySamples + L" mds=" + mMaxDelaySamples + L" speed=" + mDelayConfig.getSpeedUpPerc() + L" tempo=" + (mTempoChangeHQ ? L"HQ" : L"LQ"));
		}
		...
		{
			if(mDebugInfo)
				EGLOG(DebugLevel::ALL, mLogPrefix + L" "  + header + L" "  + mUpperTargetDelaySamples + L" " + lagSamples + L" " + playSamplePos + L" " + mWriteSamplePos + L" " + (playSamplePos + mTargetDelaySamples));
		}
		...
		template<typename T>
		void AudioOutDelayControl<T>::service(void)
		{
			if(mStarted)
			{
				// cache PlayerPos
		...
		void AudioOutDelayControl<T>::push(Buffer<T> frame)
		{
			...
			{
				EGLOG(DebugLevel::ERRORS, mLogPrefix + L" audio frames are not of size: " + frame.getSize() + " != " + mFrameSize);
				return;
			}
		...
		template<typename T>
		void AudioOutDelayControl<T>::flush(void)
		{
			processFrame(mEmptyBuffer, mPlaySamplePos, true);
		}
